...
- **lake**: トークンが無いときの pull のエラーで、原因を先に言う
...
- どのハンドルのデータセットかをサーバーに聞いてから attach する

- レビュー指摘を直す（エラー経路・同名衝突・本番条件の検証）

- 同じデータセットの別表記を冪等にする

- 存在しない detach を案内せず、最初の表記を残す
...
- **secret**: 認証情報の発行が一度失敗しただけでビルドを終わらせない
...
- **s3**: 一時的な失敗でリクエストを送り直す

- **s3**: 分割アップロードの公開が返す答えを取りこぼさない

- **s3**: 断られた認証情報を取り直してから送り直す

- **s3**: 署名エラーとして断られた認証情報も取り直す

### Chores

- **tools**: プローブに handle 付きでデータセットを渡せるようにする

- **tools**: 凍結されたデータセットにプローブを向けたら理由を言う

- **tools**: 分割アップロードの条件をストレージ自身に聞く

### Features

- **lake**: 公開するカタログの DuckLake 形式を push 直前に検査する

- **push**: カタログをディスクから分割して上げる

- **s3**: 分割したアップロードにも条件を付けられるようにする
